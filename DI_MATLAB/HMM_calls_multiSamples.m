clc
clear
addpath(genpath('MATLAB_Dir\\MATLAB\\required_modules\\HMMall\\')); 
addpath(genpath('MATLAB_Dir\\MATLAB\\required_modules\\pmtk3-2april2011\\')); 
addpath(genpath('MATLAB_Dir\\MATLAB\\required_modules\\order'));

D = dir('InputDir\\*.di');
for i = 1 : 样本数量
    clearvars -EXCEPT i D
    D = dir('InputDir\\*.di');
    filename = ['InputDir\\' D(i).name];
    filename
    minaic = 100000000;
    data = load(filename);
