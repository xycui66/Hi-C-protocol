...
mkdir chr_split/
for i in {1..21};do awk -F "\t" -v chr=chr${i} '$1==chr' SAMPLE_hmm_7colfile > chr_split/chr${i}_SAMPLE_hmm_7colfile;done
...
perl $domainCall/7hmm-state_domains.pl > chr_split/Raw_TAD/40000.${i}.tad;
done
...
#虽然该样本来源雄性小鼠，但由于此数据为小测，深度较低，chrY可能会报错，可忽略
~~~
