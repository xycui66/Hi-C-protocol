...
~~~
nohup bash $hicpro2juicebox -i $InputValidPairs -g $Hic_ann/chrom_mm10.sizes -j $JUICERJAR -r $Hic_ann/mm10_MboI.bed -t Juicebox_OutDir 2>&1 &

nohup java -jar $JUICERJAR pre -r 10000,20000,40000,50000,75000,100000,300000,500000,1000000,4000000 \
-f Juicebox_OutDir/*resfrag.juicebox Juicebox_OutDir/*.pre_juicebox_sorted Juicebox_OutDir/SAMPLE.hic $Hic_ann/chrom_mm10.sizes 2>&1
~~~
...
~~~
python $HiCPlotter -f $Input_matrix -bed $Hic_ann/${Res}_mm10.bed -n OutputLabel -o OutputPrefix -chr $chr -tri 1 -r $Res -hmc 1 -ptr 1
~~~
#### 常用参数
+ -f指定输入的Hi-C交互矩阵
+ -n指定交互矩阵热图的标题名称
