...
awk -v OFS="\t" -v intra=${o/matrix/intra} -v inter=${o/matrix/inter} 
'NR==FNR{if($4>a[$1]){a[$1]=$4}} NR>FNR{for(i in a){if($1<=a[i]&&$2>a[i]){print $0 > inter;next}}; print $0 > intra}' 
$Hic_ann/40000_mm10.bed $i & 
done
~~~
### 将所有的等距的互作数加和，距离以40K为单位记录
~~~
for j in *intra;do 
awk -v OFS="\t" -v OFS="\t" '{a[$2-$1]+=$3} END{for(i in a)print i,a[i]}' $j| 
sort -k1n,1 >${j/intra/tab} & done
~~~
### Rscript
~~~
